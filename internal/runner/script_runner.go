...
type ScriptRunner interface {
	...
	Run(ctx context.Context) (mngdom.MangaResponse, error)
}
...
func (sr *MangaScriptRunner) Run(ctx context.Context) (mngdom.MangaResponse, error) {
	...
	if err != nil {
		return mngdom.MangaResponse{}, fmt.Errorf("台本の生成に失敗したのだ: %w", err)
	}
...
func (sr *MangaScriptRunner) parseResponse(raw string) (mngdom.MangaResponse, error) {
	...
	if err := json.Unmarshal([]byte(rawJSON), &manga); err != nil {
		return mngdom.MangaResponse{}, fmt.Errorf("JSONのパースに失敗したのだ: %w", err)
	}
	return manga, nil
}
